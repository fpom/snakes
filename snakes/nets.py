...
class NetElement (object) :
    ...
    def lock (self, name, owner, value=None) :
        ...
        @param owner: any value, usually the object instance that
          locks the attribute
        @type owner: C{object}
        @param value: the value to assign to the attribute of C{None}
...
#             self._locks[name] = owner
    ...
    def unlock (self, name, owner, remove=False, value=None) :
        """Release a locked attribute, making it assignable or
        deletable from everywhere.

        @param name: the name of the attribute
        @type name: C{str}
        @param owner: the value that was used to lock the attribute
        @type owner: C{object}
        @param remove: C{True} if the attribute as to be deleted once
          unlocked, C{False} to keep it in the object
        @type remove: C{bool}
        @param value: the value to assign to the attribute are C{None}
          to do nothing special
        @type value: C{object} or C{None}
        """
        if "_locks" not in self.__dict__ :
            self.__dict__["_locks"] = {"_locks" : self}
...
#             raise ConstraintError, "'%s' not locked" % name
    def __setattr__ (self, name, value) :
        ...
        Check whether the assignment is allowed or not, the owner
        may re-assign the attribute by using C{lock} with its
        parameter C{value} instead of a direct assignment.
    ...
    def __delattr__ (self, name) :
        ...
        Check whether the deletion is allowed or not, the owner
        may delete the attribute by using C{unlock} with its
        parameter C{remove} set to C{True} instead of a direct
        deletion.
...
class Token (NetElement) :
    ...
    def __eq__ (self, other) :
        ...
        @type other: C{Token}
        @return: C{True} if token values are equal, C{False} otherwise
        @rtype: C{bool}
        """
        try :
    ...
    def __ne__ (self, other) :
        ...
        @type other: C{Token}
        @return: C{True} if token values differ, C{False} otherwise
        @rtype: C{bool}
        """
        return not self.__eq__(other)
    def __hash__ (self) :
...
class BlackToken (Token) :
    ...
    def __repr__ (self) :
        ...
        Since C{snakes.nets} sets C{dot = BlackToken()}, the string
        representation is C{'dot'}.
...
class ArcAnnotation (NetElement) :
    ...
    A class attribute C{input_allowed} is set to C{True} or C{False}
    ...
    def replace (self, old, new) :
        ...
        annotation if it is not the same as C{old} otherwise it
        returns C{new}. Composite annotations will replace there
        components.
        ...
        @type old: C{ArcAnnotation}
        @param new: the annotation to use instead of C{old}
        @type new: C{ArcAnnotation}
        """
        if old == self :
            return new
        else :
...
class Value (ArcAnnotation) :
    ...
    @classmethod
    def __pnmlload__ (cls, tree) :
        """Create a C{Value} from a PNML tree
    ...
    def __eq__ (self, other) :
        ...
        @type other: C{Value}
        @return: C{True} if encapsulated values are equal, C{False} otherwise
        @rtype: C{str}
        """
        try :
            return self.value == other.value
    ...
    def __hash__ (self) :
        return hash(self.value)
    def bind (self, binding) :
        """Return the value evaluated through C{binding} (which is the
        value itself).
    ...
    def modes (self, values) :
        """Return an empty binding (no binding needed for values) iff
        the value is in C{values}, raise ModeError otherwise.
...
class Variable (ArcAnnotation) :
    ...
    def __init__ (self, name) :
        """Variable names must start with a letter and may continue
        with any alphanumeric character.
    ...
    def __pnmldump__ (self) :
        """Dump a C{Variable} as a PNML tree
    ...
    @classmethod
    def __pnmlload__ (cls, tree) :
        """Create a C{Variable} from a PNML tree
    ...
    def modes (self, values) :
        """Return the the list of substitutions mapping the name of
        the variable to each value in C{values}.
    ...
    def substitute (self, binding) :
        """Change the name according to C{binding}.
    ...
    def vars (self) :
        ...
        @rtype: C{list} holding a single C{str}
        """
        return [self.name]
    def __eq__ (self, other) :
        """Test for equality
        ...
        @return: C{True} if variables have the same name, C{False} otherwise
        @rtype: C{bool}
        """
        try :
...
class Expression (ArcAnnotation) :
    ...
    def __pnmldump__ (self) :
        """Dump an C{Expression} as a PNML tree
    ...
    @classmethod
    def __pnmlload__ (cls, tree) :
        """Create an C{Expression} from a PNML tree
    ...
    def bind (self, binding) :
        """Evaluate the expression through C{binding}.
    ...
    def __call__ (self, binding) :
        "Returns the value from C{bind} (but not encapsulated)."
    ...
    def __and__ (self, other) :
        """
        Implement C{&} to perform C{and} between two expressions.

        It is not checked whether we combine boolean expressions or not.
        ...
        @type other: C{snakes.nets.Expression}
        @return: an expression
        @rtype: C{snakes.nets.Expression}
        """
        if self._true :
            return other.copy()
        elif other._true :
    ...
    def __or__ (self, other) :
        """
        Implement C{|} to perform C{or} between two expressions.
    ...
    def __invert__ (self) :
        """
        Implement C{~} to perform C{not}.

        It is not checked whether we negate a boolean expression or not.
...
def let (**args) :
    ...
    Assignement takes place when method C{bind} of the expression is
    called. Assigned variables are then stored in the C{Substitution}
    passed to C{bind}.

    Use with care: sides effects are nasty tricks, you may get
    unexpected results while playing with C{let}.
    ...
    @param args: a list of C{name=value} to assign
    @return: C{True} if assignment could be made, C{False} otherwise
    @rtype: C{bool}
    """
    try :
...
class MultiArc (ArcAnnotation) :
    """A collection of other annotations, allowing to consume or
    ...
    def __pnmldump__ (self) :
        """Dump a C{MultiArc} as a PNML tree
    ...
    @classmethod
    def __pnmlload__ (cls, tree) :
        """Create a C{MultiArc} from a PNML tree
    ...
    def replace (self, old, new) :
        ...
        With C{MultiArc}, replaces each occurrence of C{old} by C{new}
...
class Test (ArcAnnotation) :
    ...
    @classmethod
    def __pnmlload__ (cls, tree) :
        """Create a C{Test} from a PNML tree
    ...
    def replace (self, old, new) :
        ...
        With C{Test}, always returns C{Test(new)}
...
class Inhibitor (Test) :
    ...
    def __pnmldump__ (self) :
        """Dump a C{Inhibitor} as a PNML tree
    ...
    @classmethod
    def __pnmlload__ (cls, tree) :
        """Create a C{Inhibitor} from a PNML tree
    ...
    def bind (self, binding) :
        """Return the value of the annotation evaluated through
        C{binding}, or raise C{ValueError} of this binding does not
...
class Node (NetElement) :
    ...
    This class is abstract and should not be instanciated, use
    C{Place} or C{Transition} as concrete nodes."""
    def rename (self, name) :
...
class Place (Node) :
    ...
    def __init__ (self, name, tokens=[], check=None) :
        ...
        C{tokens} may be a single value or an iterable object.
        C{check} may be C{None} (any token allowed) or a type from the
        module typing.
        ...
        @type tokens: any collection like C{MultiSet}, C{set}, C{list},
          C{tuple}, ...
        @param check: a constraint on the tokens allowed in the place
          (or C{None} for no constraint)
        @type check: a type from the module C{typing}
        """
        self.name = name
    ...
    def copy (self, name=None) :
        ...
        @param name: if not C{None}, the name of the copy
        @type name: C{str}
    ...
    def checker (self, check=None) :
        ...
        @param check: the new constraint for the place or C{None} to retreive
          the current constraint
        @type check: C{None} or a type from the module C{typing}
        @return: the current constraint if C{check} is C{None} or C{None}
          otherwise
        @rtype: a type from the moduleC{typing} or C{None}
        """
        if check is None :
            return self._check
        else :
    ...
    def __contains__ (self, token) :
        ...
        @type token: any type
        @return: C{True} if C{token} is held by the place, C{False} otherwise
        @rtype: C{bool}
        """
    ...
    def is_empty (self) :
        ...
        @return: C{True} if the place is empty, C{False} otherwise
        @rtype: C{bool}
        """
        return self.tokens.size() == 0
    def check (self, tokens) :
        """Check if the 'tokens' are allowed in the place. An
        exception C{ValueError} is raised whenever a forbidden token
        is encountered.
    ...
    def __repr__ (self) :
        ...
        @return: a string suitable to C{eval} representing the place
        @rtype: C{str}
        """
    ...
    def remove (self, tokens) :
        ...
        @param tokens: a collection of tokens to be removed from the place
        @type tokens: any type, iterable types (except C{str}) will be
          considered as collections of values
        """
...
class Transition (Node) :
    ...
    def __init__ (self, name, guard=None) :
        ...
        If C{guard} is C{None}, C{True} is assumed instead.
        ...
        @type guard: C{Expression} with a boolean value
        """
        self._input = {}
        self._output = {}
        if guard is None :
    ...
    def copy (self, name=None) :
        ...
        @param name: if not C{None}, the name if of the copy
        @type name: C{str}
    ...
    def __repr__ (self) :
        ...
        @return: a string suitable to C{eval} representing the transition
        @rtype: C{str}
        """
    ...
    def add_input (self, place, label) :
        """Add an input arc from C{place} labelled by C{label}.
    ...
    def remove_input (self, place) :
        """Remove the input arc from C{place}.
    ...
    def remove_output (self, place) :
        """Remove the output arc to C{place}.
    ...
    def _check (self, binding, tokens, input) :
        ...
        If C{tokens} is C{True}, check whether the flow of tokens can
        be provided by the input places. If C{input} is C{True}, check
        if the evaluation of the inputs arcs respect the types of the
        ...
        @type binding: C{data.Substitution}
        ...
        @type input: C{bool}
        @return: C{True} if the binding is suitable, C{False} otherwise
        """
    ...
    def activated (self, binding) :
        """Check if C{binding} activates the transition.

        This is the case if the guard evaluates to C{True} and if the
        ...
        @param binding: a valuation of the variables on the transition
        @type binding: C{Substitution}
        """
        return self._check(binding, False, True)
    def enabled (self, binding) :
        """Check if C{binding} enables the transition.
    ...
    def vars (self) :
        """Return the set of variables involved in the guard, input
        and output arcs of the transition.
    ...
    def substitute (self, binding) :
        ...
        C{binding} is used to substitute the guard and all the labels
        ...
        @param binding: a substitution from variables to variables (not values)
        @type binding: C{Substitution}
        """
    ...
    def modes (self) :
        ...
        bindings that I{activate} a transitions. However, this list is
        generally infinite so we retricted ourselves to I{actual
        modes}, taking into account the present tokens.
        ...
        return result
    def flow (self, binding) :
        """Return the token flow for a firing with C{binding}.

        The flow is represented by a pair C{(in, out)}, both being
        instances of the class C{Marking}.
        ...
        @param binding: a substitution from variables to values (not variables)
        @type binding: C{Substitution}
        @return: a pair of marking to be respectively consumed or
          produced by the firing of the transition with C{binding}
        @rtype: a C{tuple} of two C{Marking}
        """
...
class Marking (hdict) :
    ...
    This is basically a C{snakes.hashables.hdict} mapping place names
    to multisets of tokens.

    The parameters for the constructor must be given in a form
    suitable for initialising a C{hdict} with place names as keys and
    multisets as values. Places not given in the marking are assumed
    empty. A C{Marking} object is independent of any Petri net and so
    ...
    def __call__ (self, place) :
        """
        Return the marking of C{place}.

        The empty multiset is returned if C{place} is not explicitely
    ...
    def __eq__ (self, other) :
        ...
        @param other: another marking
        @type other: C{Marking}
        @return: C{True} if the markings are equal, C{False} otherwise
        @rtype: C{bool}
        """
    ...
    def __ne__ (self, other) :
        ...
        @param other: another marking
        @type other: C{Marking}
        @return: C{True} if the markings differ, C{False} otherwise
        @rtype: C{bool}
        """
        return not (self == other)
    def __ge__ (self, other) :
        """
        Test if the marking C{self} is greater than or equal to
        C{other}.

        This is the case when any place in C{other} is also in C{self}
        ...
        @param other: another marking
        @type other: C{Marking}
        @return: C{True} if C{self >= other}, C{False} otherwise
        @rtype: C{bool}
        """
        for place in other :
    ...
    def __gt__ (self, other) :
        """
        Test if the marking C{self} is strictly greater than C{other}.

        This is the case when any place in C{other} is also in C{self}
        and either one place in C{other} is marked with a smaller
        multiset of tokens or C{slef} has more places than C{other}.
    ...
    def __le__ (self, other) :
        """
        Test if the marking C{self} is smaller than or equal to C{other}.

        This is the case when any place in C{self} is also in C{other}
        and is marked with a smaller or equal multiset of tokens.
    ...
    def __lt__ (self, other) :
        """
        Test if the marking C{self} is strictly smaller than C{other}.

        This is the case when any place in C{self} is also in C{other}
        and either one place in C{self} marked in self with a strictly
        smaller multiset of tokens or C{other} has more places than
        C{self}.
...
class PetriNet (object) :
    ...
    As soon as nodes are added to a C{PetriNet}, they are handled by
    name instead of by the C{Place} or C{Transition} instance. For
    instance:
    ...
    def __pnmldump__ (self) :
        """Dump a C{PetriNet} as a PNML tree
    ...
    @classmethod
    def __pnmlload__ (cls, tree) :
        """Create a C{PetriNet} from a PNML tree
        ...
        return result
    def __repr__ (self) :
        """Return a string suitable for C{eval} to represent the net.
    ...
    def has_place (self, name) :
        """Check if there is a place called C{name} in the net.
        ...
        @type name: C{str}
        @return: C{True} if a place called C{name} is present in the
          net, C{False} otherwise
        @rtype: C{bool}
        """
        return name in self._place
    def has_transition (self, name) :
        """Check if there is a transition called C{name} in the net.
        ...
        @type name: C{str}
        @return: C{True} if a transition called C{name} is present in the
          net, C{False} otherwise
        @rtype: C{bool}
        """
    ...
    def has_node (self, name) :
        ...
        @type name: C{str}
        @return: C{True} if a node called C{name} is present in the
          net, C{False} otherwise
        @rtype: C{bool}
        """
        return name in self._node
    def __contains__ (self, name) :
        """C{name in net} is a shortcut for C{net.has_node(name)}
    ...
    def declare (self, statements, locals=None) :
        """Execute C{statements} in the global dictionnary of the net.

        This has also on the dictionnarie of the instances of
        C{Expression} in the net (guards of the transitions and labels
        on the arcs) so the declarations have an influence over the
        elements embedded in the net. If C{locals} is given, most of
        ...
        @param statements: a Python instruction suitable to C{exec}
        @type statements: C{str} or C{code}
        @param locals: a C{dict} used as locals when C{statements} is
          executed, or C{None}
        @type locals: C{dict} or something compatible
        """
    ...
    def place (self, name=None) :
        """Return one (if C{name} is not C{None}) or all the places.
        ...
        @param name: the name of the place to retrieve or C{None} to
          get the list of all the places in the net
        @type name: C{str} or C{None}
        @return: a place whose name is C{name} or a list of places
    ...
    def transition (self, name=None) :
        ...
        @param name: the name of the transition to retrieve or C{None} to
          get the list of all the transitions in the net
        @type name: C{str} or C{None}
        @return: a transition whose name is C{name} or a list of transitions
    ...
    def node (self, name=None) :
        ...
        @param name: the name of the node to retrieve or C{None} to
          get the list of all the nodes in the net
        @type name: C{str} or C{None}
        @return: a node whose name is C{name} or a list of nodes
        @rtype: C{Transition} or C{Place}, or a C{list} of
    ...
    def add_output (self, place, trans, label) :
        """Add an output arc between C{place} and C{trans} (nodes names).
    ...
    def remove_output (self, place, trans) :
        """Remove an output arc between C{place} and C{trans} (nodes names).
        ...
        if hasattr(l, "globals") :
            l.globals.detach(self.globals)
    def pre (self, nodes) :
        """Return the set of nodes names preceeding C{nodes}.

        C{nodes} can be a single node name ot a list of nodes names.
    ...
    def _set_marking (self, marking) :
        ...
        thus use C{set_marking} unless you will have no error.
    ...
    def copy_place (self, source, targets) :
        """Make copies of the C{source} place (use place names).
        ...
        @type targets: C{str} or C{list} of C{str}
        """
    ...
    def copy_transition (self, source, targets) :
        """Make copies of the C{source} transition (use transition names).
    ...
    def merge_places (self, target, sources) :
        """Create a new place by merging those in C{sources}.
        ...
        @param sources: the list of places names to be merged (or a
          single place name)
        @type sources: C{list} of C{str} (or C{str})
        """
    ...
    def merge_transitions (self, target, sources) :
        """Create a new transition by merging those in C{sources}.

        Guards are 'and'ed and arcs labels are joinded into
        multi-arcs, the sources transitions are not removed. Use
        transitions names.
        ...
        @type target: C{str}
        @param sources: the list of transitions names to be merged (or a
...
class StateGraph (object) :
    ...
    def successors (self, state=None) :
        ...
        @rtype: C{dict} mapping non-negative C{int} to C{tuple}
          holding a C{str} and a C{Substitution}
        """
        if state is None :
            state = self.current()
        self._process(state)
    ...
    def predecessors (self, state=None) :
        ...
        The returned value is as in C{successors}. Notice that if the
        ...
        @return: the dictionnary of predecessors and transitions to them
        @rtype: C{dict} mapping non-negative C{int} to C{tuple}
          holding a C{str} and a C{Substitution}
        """
        if state is None :
    ...
    def __iter__ (self) :
        ...
        B{Warning:} the net may have an infinite state graph, which is
        not checked. So you may enter an infinite iteration.
    ...
    def _build (self, stop=None) :
        ...
        B{Warning:} this may be infinite! No check of this is
        performed.
    ...
    def completed (self) :
        ...
        @return: C{True} if the graph has been completely computed,
          C{False} otherwise
        @rtype: C{bool}
        """
